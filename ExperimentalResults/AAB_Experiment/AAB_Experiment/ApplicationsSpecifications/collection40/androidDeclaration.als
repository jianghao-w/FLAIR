...
assert intentHijacking{
	no disj c1, b: Component, i1:Intent, 
	disj d1, dIntent:DetailedPath |  i1.sender = c1 && 
	no i1.component &&
	b in newIntentResolver[i1] &&
	no c1.app & b.app &&
	c1.app in existingApps.apps &&
	not (b.app in existingApps.apps) &&
	dIntent in i1.detailedPaths && dIntent.sink = IPC && dIntent.source in SIGResource && 	
	b in Activity  && 
	#b.detailedPaths = 1  && 
	b.detailedPaths = d1  && 
	d1.source = IPC  && 
...
assert  appCollusion{
  no disj cmp1, cmp2: Component, i:Intent, 
    disj d1,d2: DetailedPath| 
    cmp2 in newIntentResolver[i] &&
    i.sender in cmp1 &&
    d1 in i.detailedPaths && 
    d2 in cmp2.detailedPaths && 
    usesPermissions[cmp1.app] != usesPermissions[cmp2.app] &&
    cmp1.app != cmp2.app
}
...
assert intentSpoofing{
	no 	disj c2,b: one Component,	i2: Intent,	dComponent: one DetailedPath | 
	i2.sender = b &&
	c2 in setExplicitIntent[i2] &&
	no c2.app & b.app &&
	c2.app in existingApps.apps &&
	not (b.app in existingApps.apps) &&
	some i2.detailedPaths && i2.detailedPaths.sink = IPC && i2.detailedPaths.source = NEW	&&
	dComponent in c2.detailedPaths && dComponent.source = IPC && 
	dComponent.sink in SIGResource  &&
	b in Activity &&
	no b.detailedPaths &&
	no b.intentFilter
}
